...
const realInput = `Game 1: 13 red, 18 green; 5 green, 3 red, 5 blue; 5 green, 9 red, 6 blue; 3 blue, 3 green
...
const games = realInput.split("\n").map((l) => {
  const [game, handsInput] = l.split(":");
  const id = +game.split(" ")[1];
  const hands = handsInput.split(";").map((h) =>
    [...h.matchAll(/(\d+)\s(blue|red|green)/g)].flatMap((matches) => {
      const [number, color] = matches.slice(1, 3);
...
const possibleGames = games.filter((g) =>
  g.hands.every((h) =>
    h.every(({ number, color }) => {
      return availableCubes[color] >= number;
    })
  )
);
...
console.log(possibleGames.map(({ id }) => id).reduce((sum, id) => sum + id, 0));
    ...
    .map((g) =>
      colors
        .map((c) =>
          Math.max(
            ...g.hands.flatMap((h) =>
              h.map((cube) => (cube.color === c ? cube.number : 0))
            )
          )
        )
