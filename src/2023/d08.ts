...
export function run() {
  console.log('Day 8');
  const { rules, directions } = parse(realInput);
  let currentElement = rules.find(({ element }) => element === 'AAA')!;
  let count = 0;
  for (let index = 0; index < directions.length; index = (index + 1) % directions.length) {
    const direction = directions[index];
    currentElement = rules.find((r) => r.element === currentElement[direction])!;
    count++;
    if (currentElement.element === 'ZZZ') {
      console.log('Solved part 1: ', count);
      break;
    }
  }
...
function travelFromAToZ(start: Rule, rules: Rule[], directions: Array<'L' | 'R'>): number {
  ...
  for (let index = 0; index < directions.length; index = (index + 1) % directions.length) {
    ...
    currentElement = rules.find((r) => r.element === currentElement[direction])!;
    count++;
    if (currentElement.element.endsWith('Z')) {
      return count;
    }
    if (count > 50000) {
      break;
...
function parse(s: string) {
  const lines = s.split('\n');
  const directions = lines[0].split('') as Array<'R' | 'L'>;

  const rules = lines
    .slice(2)
    .map((l) => l.split('=').map((s) => s.trim()))
    .map(([element, leftRight]) => {
      const L = leftRight.slice(1, 4);
      const R = leftRight.slice(6, 9);
      return { element, L, R };
    });
