...
export function run() {
  ...
  const numbers = lines
    .flatMap((l, y) => {
      const match = [...l.matchAll(/\d+/g)];
      return match.length ? match.map((m) => MachinePart.create(m.index!, y, m[0])) : [];
    })
  ...
  for (let y = 0; y < matrix.length; y++) {
    ...
    for (let x = 0; x < row.length; x++) {
      const el = row[x];
      if (isSymbol(el)) {
        const nums = numbers.filter((n) => isAdjacent([x, y], n));
        nums.forEach((n) => partNumbers.add(n));
        if (el === '*' && nums.length === 2) {
          gearRatios.push(nums[0].number * nums[1].number);
        }
      }
    }
  }
...
const isAdjacent = ([x, y]: [number, number], partNumber: { x: number; y: number; length: number }) => {
  if (Math.abs(y - partNumber.y) > 1) return false;
  return x + 1 >= partNumber.x && x <= partNumber.x + partNumber.length;
};

const isDigit = (c: string) => c.match(/\d/);
const isSymbol = (c: string) => c !== '.' && !isDigit(c);

class MachinePart {
  private static cache = new Map<string, MachinePart>();
  static create(x: number, y: number, n: string) {
    const key = `${x},${y}`;
    const part = MachinePart.cache.get(key) ?? new MachinePart(x, y, n.length, +n);
    MachinePart.cache.set(key, part);
    return part;
  }
  private constructor(
