...
export function run() {
  ...
  const intersections = rucksacks
    .map((contents) =>
      contents
        .slice(0, contents.length / 2)
        .uniq()
        .intersect(contents.slice(contents.length / 2).uniq())
    )
    .flat();
  console.log(intersections.map(priority).sum());
}
...
export function run2() {
  ...
  const intersections = groups.map((g) => g[0].intersect2(g[1], g[2]).uniq()).flat();
  console.log(intersections.map(priority).sum());
}

function isUpper(l: string): boolean {
  return l.toUpperCase() === l;
}

function priority(letter: string): number {
  return letter.charCodeAt(0) - (isUpper(letter) ? 38 : 96);
}

function input() {
  return `gfWpjRRQffQGCHHJsGqjsj
